...
import { getHandGestureAnalysis } from '@/app/actions';
...
export default function HandGestureDemo() {
  ...
  const prevCoords = useRef<{ x?: number; y?: number }>({ x: undefined, y: undefined });
  ...
  const captureAndAnalyze = useCallback(async () => {
    ...
    try {
      const res = await getHandGestureAnalysis({ 
          photoDataUri: dataUri,
          prevWristX: prevCoords.current.x,
          prevWristY: prevCoords.current.y,
        });

      if (res.success && res.data) {
        setResult({
            handState: res.data.handState,
            movement: res.data.movement
        });
        prevCoords.current = { x: res.data.wristX, y: res.data.wristY };
      } else {
         toast({
          variant: 'destructive',
          title: 'AI Error',
    ...
    <Card className="h-full bg-card border-accent/20 overflow-hidden glow-accent shadow-2xl shadow-accent/10">
      <CardHeader>
        <div className="flex items-start gap-4">
          ...
          <div>
            <CardTitle className="text-2xl font-headline text-glow-accent">Hand Gesture Recognition Demo</CardTitle>
            <CardDescription className="pt-2">Allow camera access to have the AI analyze your hand gestures and movements in real-time.</CardDescription>
          </div>
        </div>
      </CardHeader>
        ...
        {result ? (
            <div className="p-4 bg-muted rounded-lg space-y-4 text-center animate-in fade-in-50">
                ...
                <div>
                    ...
                    <p className="text-2xl font-bold capitalize text-accent text-glow-accent">{result.movement}</p>
                </div>
            </div>
        ) : (
         ...
         <p className="text-xs text-muted-foreground text-center italic">
            Disclaimer: This AI demo is for educational purposes. Analysis may not be 100% accurate.
         </p>
      </CardContent>
    </Card>
  );
